...
const MODULE_ADDRESS = "340b3ba2b30cf9a9b7380492fb5c3281d5ff1b8078623dbf9a247b1c9d1ed2cc";
...
function App() {
  ...
  const storeMovie = async () => {
    ...
    //   };
      ...
      setStatus(Transaction failed ❌: ${error.message || JSON.stringify(error)});
    }
  };
    ...
    <div>
      ...
      {connected ? (
        <>
          ...
          <div>
            <input type="text" placeholder="Enter Movie Title" value={movieTitle} onChange={(e) => setMovieTitle(e.target.value)} />
            <input type="text" placeholder="Enter Movie Date" value={movieDate} onChange={(e) => setMovieDate(e.target.value)} />
            <input type="text" placeholder="Enter Movie Location" value={movieLocation} onChange={(e) => setMovieLocation(e.target.value)} />
            <input type="text" placeholder="Enter Movie Price" value={moviePrice} onChange={(e) => setMoviePrice(e.target.value)} />
            <input type="text" placeholder="Enter Movie Image URL" value={movieImageUrl} onChange={(e) => setMovieImageUrl(e.target.value)} />
          ...
          <ul>
            {movies.length > 0 ? (
              movies.map((movie, index) => (
                <li key={index}>{new TextDecoder().decode(new Uint8Array(movie))}</li>
              ))
            ) : (
              <p>No movies found.</p>
            )}
